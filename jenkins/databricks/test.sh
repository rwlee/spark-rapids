...
if [ -d "$LOCAL_JAR_PATH" ]; then
    if [[ $TEST_MODE == "DEFAULT" ]]; then
        ...
        LOCAL_JAR_PATH=$LOCAL_JAR_PATH bash $LOCAL_JAR_PATH/integration_tests/run_pyspark_from_build.sh -k  --runtime_env="databricks" --test_type=$TEST_TYPE
    ...
    if [[ $TEST_MODE == "DEFAULT" ]]; then
        ...
        TEST_NAME=$1 bash /home/ubuntu/spark-rapids/integration_tests/run_pyspark_from_build.sh --runtime_env="databricks" -k $TEST_NAME --test_type=$TEST_TYPE

        ## Run cache tests
        if [[ "$IS_SPARK_311_OR_LATER" -eq "1" ]]; then
            PYSP_TEST_spark_sql_cache_serializer=${PCBS_CONF} \
            TEST_NAME=$1 bash /home/ubuntu/spark-rapids/integration_tests/run_pyspark_from_build.sh --runtime_env="databricks" -k $TEST_NAME --test_type=$TEST_TYPE -k cache_test
        fi
    fi
    ...
    if [[ "$TEST_MODE" == "CUDF_UDF_ONLY" ]]; then
        ...
        SPARK_SUBMIT_FLAGS="$SPARK_CONF $CUDF_UDF_TEST_ARGS" TEST_PARALLEL=1 \
            TEST_NAME=$1 bash /home/ubuntu/spark-rapids/integration_tests/run_pyspark_from_build.sh --runtime_env="databricks" -k $TEST_NAME -m "cudf_udf" --cudf_udf --test_type=$TEST_TYPE
    fi

    if [[ "$TEST_MODE" == "DEFAULT" || "$TEST_MODE" == "ICEBERG_ONLY" ]]; then
        ## Run Iceberg tests
        SPARK_SUBMIT_FLAGS="$SPARK_CONF $ICEBERG_CONFS" TEST_PARALLEL=1 \
            TEST_NAME=$1 bash  /home/ubuntu/spark-rapids/integration_tests/run_pyspark_from_build.sh --runtime_env="databricks" -k $TEST_NAME -m iceberg --iceberg --test_type=$TEST_TYPE
    fi
