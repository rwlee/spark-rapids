...
def _get_overflow_error(bracket, tail):
    ...
    err_mess = ('' if is_before_spark_340() and not \
        is_databricks113_or_later()  else '[' + bracket + '] ') + tail
...
def test_multiplication_ansi_overflow():
    assert_gpu_and_cpu_error(
        ...
        ansi_enabled_conf,
        error_message=_get_overflow_error('NUMERIC_VALUE_OUT_OF_RANGE', ''))
...
def test_unary_minus_ansi_overflow(data_type, value):
    ...
    assert_gpu_and_cpu_error(
            ...
            conf=ansi_enabled_conf,
            error_message=_get_overflow_error(''))
...
def test_abs_ansi_overflow(data_type, value):
    ...
    assert_gpu_and_cpu_error(
            ...
            conf=ansi_enabled_conf,
            error_message=_get_overflow_error('ARITHMETIC_OVERFLOW', 'long overflow.'))
...
def test_add_overflow_with_ansi_enabled(data, tp, expr):
    if isinstance(tp, IntegralType):
        assert_gpu_and_cpu_error(
            ...
            conf=ansi_enabled_conf,
            error_message=_get_overflow_error('INTERVAL_ARITHMETIC_OVERFLOW', 'long overflow.'))
