...
object SparkShimImpl extends Spark331PlusShims {
  ...
  override def checkOverflowRule: ExprRule[_ <: Expression] = null

  def promotePrecisionRule: ExprRule[_ <: Expression] = null

  override def ignoreTimeZoneCastBase(e: Expression): Expression = e match {
    case c: Cast if c.timeZoneId.nonEmpty && !c.needsTimeZone => // Do we need this?
      c.withTimeZone(null)
